...
class Args:
    ...
    # -- factuality reward
    apply_factuality_reward: bool = False
    """whether to apply factuality reward"""
    factuality_reward: float = 1.0
    """the reward value for factuality reward"""
    factuality_score_type: Literal["fact-strict-score", "fact-score"] = None
    """the type of factuality score to use"""
    factuality_config_kwargs: str = None
    # factuality_decomposition_model_name: str = "gpt-4.1-mini"
    # """the model name for factuality decomposition"""
    # factuality_verification_model_name: str = "gpt-4.1-mini"
    # """the model name for factuality verification"""
    factuality_metric: str = "default"
    """the metric to use for factuality reward"""
    factuality_cache_dir: Optional[str] = None
    """the cache directory for factuality reward"""
    factuality_additional_reward: Optional[str] = "general-quality_ref"
    """the additional reward dataset to use for factuality reward"""
...
def apply_factuality_reward(
    decoded_prompts: List[str],
    decoded_responses: List[str],
    documents: List[str],
    reward_mult: int = 1.0,
    factuality_score_type: str = "fact-strict-score",
    factuality_config_kwargs: dict = None,
    metric: str = "precision",
    cache_dir: Optional[str] = None,
):
    # from fact_eval.fact_utils.factuality_scorer import ClaimWiseFactualityScorer
    # scorer = ClaimWiseFactualityScorer(
    #     decomposition_model_name=decomposition_model_name,
    #     verification_model_name=verification_model_name,
    #     search_num_processes=1,
    #     cache_dir=cache_dir,
    #     metric=metric,
    # )
    # fs_dict = scorer.get_score(decoded_prompts, decoded_responses, documents, return_metadata=False)
    ...
    # add time stamp to cache dir (YYYYMMDD-HHMMSS)
    cache_dir_ts = os.path.join(cache_dir, datetime.now().strftime("%Y%m%d-%H%M%S"))
    os.makedirs(cache_dir_ts, exist_ok=True)

    if factuality_score_type == "fact-strict-score":
        from fact_eval.scorer.factstrictscore import FactStrictScorer, FactStrictScoreConfig
        scorer = FactStrictScorer(
            FactStrictScoreConfig(
                **factuality_config_kwargs,
                cache_dir=cache_dir_ts,
            )
        )
    elif factuality_score_type == "fact-score":
        from fact_eval.scorer.factscore import FactScorer, FactScoreConfig
        scorer = FactScorer(
            FactScoreConfig(
                **factuality_config_kwargs,
                cache_dir=cache_dir_ts,
    ...
    final_answer_list = [extract_final_answer(response) for response in decoded_responses]

    _, per_instance_results = scorer.get_score([{
        "prompt": prompt,
        "response": response,
        "docs": docs,
    } for prompt, response, docs in zip(decoded_prompts, final_answer_list, documents)])

    # rewards = []
    # for fs in fs_dict["factuality_score"]:
    #     assert isinstance(fs, float), "Factuality score should be a float"
    #     rewards.append(reward_mult * fs)
    # return rewards
    ...
    if factuality_score_type == "fact-strict-score":
        for result in per_instance_results:
            ...
            if metric == "binary":
                factuality_score_binary = 1.0 if factuality_score > 0.5 else 0.0
                rewards.append(factuality_score_binary * reward_mult)
            elif metric == "default":
                rewards.append(factuality_score * reward_mult)
            else:
                raise ValueError(f"Unknown factuality metric: {metric}")
    elif factuality_score_type == "fact-score":
        for result in per_instance_results:
            factuality_score = result.get("factuality_score", 0.0)
            num_claims = result.get("num_claims", 0)
            num_correct_claims = result.get("num_correct_claims", 0)
            if metric == "precision":
                reward = num_correct_claims / num_claims if num_claims > 0 else 0.0
            if metric == "smoothed_precision":
                reward = num_correct_claims / (num_claims + 1)
            elif metric == "smoothed_precision+detail_level":
                reward = (
                    num_correct_claims / (num_claims + 1) + 0.1 * math.log(1 + num_correct_claims) 
                )
            elif metric == 'binary':
                reward = 1.0 if num_correct_claims * 2 > num_claims else 0.0
            elif metric == "binary_one":
                reward = 1.0 if num_correct_claims == num_claims else 0.0
            elif metric == "f1@10":
                precision = num_correct_claims / num_claims if num_claims > 0 else 0.0
                recall = min(1.0, num_claims / 10)
                reward = 2 * precision * recall / (precision + recall) if num_correct_claims + num_claims > 0 else 0.0
            else:
                raise ValueError(f"Unknown factuality metric: {metric}")
            rewards.append(reward * reward_mult)
    return rewards
...
def make_reward_fn(args: Args) -> Callable:
    ...
    async def reward_fn(
        ...
        # args.apply_factuality_reward
        if args.apply_factuality_reward:
            with Timer("[Data Preparation Thread] Calculating rewards -- 🔍 Applying factuality reward"):
                # FIXME: a patch for adding factuality reward to verifiable reward
                factuality_indices = [i for i in range(len(batch.datasets)) if "factuality" in (batch.datasets[i] if isinstance(batch.datasets[i], list) else [batch.datasets[i]])]
                queries_filtered = [batch.raw_queries[i] for i in factuality_indices]
                responses_filtered = [decoded_responses[i] for i in factuality_indices]
                ground_truths_filtered = [batch.ground_truths[i] for i in factuality_indices]
                datasets_filtered = [batch.datasets[i] for i in factuality_indices]
                # json.loads ground_truths_filtered
                # assert only one item in ground_truths_filtered
                assert all(len(item) == 1 for item in ground_truths_filtered), "Only one item in ground_truths_filtered"
                documents_filtered = [json.loads(item[0])["docs"] for item in ground_truths_filtered]

                if args.factuality_config_kwargs is None:
                    factuality_config_kwargs = {}
                else:
                    try:
                        factuality_config_kwargs = json.loads(args.factuality_config_kwargs)
                    except Exception as e:
                        print(f"Warning: Invalid JSON string for factuality_config_kwargs: {args.factuality_config_kwargs}")
                        raise e
                if len(factuality_indices) > 0:
                    with Timer("[Data Preparation Thread] Calculating rewards -- 🔍 Applying factuality reward"):
                        factuality_rewards = apply_factuality_reward(
                            decoded_prompts=queries_filtered,
                            decoded_responses=responses_filtered,
                            documents=documents_filtered,
                            reward_mult=args.factuality_reward,
                            factuality_score_type=args.factuality_score_type,
                            factuality_config_kwargs=factuality_config_kwargs,
                            metric=args.factuality_metric,
                            cache_dir=args.factuality_cache_dir,
                        )
                        if len(factuality_rewards) != len(factuality_indices):
                            raise ValueError(f"{len(factuality_rewards)} != {len(factuality_indices)}")
                        for reward_idx, original_idx in enumerate(factuality_indices):
                            scores[original_idx] = factuality_rewards[reward_idx] + scores[original_idx]
                        metrics["objective/factuality_reward"] = np.array(factuality_rewards).mean()
                        metrics["objective/factuality_reward>0.5"] = (np.array(factuality_rewards) > 0.5).mean()
                ...
                for i in range(len(format_scores)):
                    if not args.only_reward_good_outputs or (good_outputs[i] and args.only_reward_good_outputs):
                        if args.additive_format_reward:
                            ...
                            scores[i] = scores[i] if format_scores[i] == 1 else 0
